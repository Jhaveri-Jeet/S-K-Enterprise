...
<body><!-- page header -->
    ...
    <section class="section">
        <div class="container">
            <div class="row items-center justify-center">
                <div class="md:col-12 lg:col-6 aos-init aos-animate" data-aos="fade-up">
                    <div class="rounded-2xl p-8 pb-10 shadow">
                        <div class="mb-6">
                            <h3 class="h5 font-semibold">Add Requirement</h3>
                        </div>
                        <form method="POST" enctype="multipart/form-data">
                            <div class="mb-6">
                                <label class="mb-2 block font-medium text-theme-dark/70" for="subject">Product Name</label>
                                <input class="form-control w-full" name="name" type="text" placeholder="Your Full Name" id="name" />
                            </div>
                            <div class="mb-6">
                                <label class="mb-2 block font-medium text-theme-dark/70" for="subject">Description</label>
                                <input class="form-control w-full" name="description" placeholder="Type Your Description" id="description" />
                            </div>
                            <div class="mb-6">
                                <label class="mb-2 block font-medium text-theme-dark/70" for="subject">Today's Date</label>
                                <input class="form-control w-full" name="todayDate" type="date" placeholder="Enter Today's Date" id="todaysDate" />
                            </div>
                            <div class="mb-6">
                                <label class="mb-2 block font-medium text-theme-dark/70" for="subject">Pieces</label>
                                <input class="form-control w-full" name="pieces" type="number" placeholder="Enter Pieces" id="pieces" />
                            </div>
                            <div class="mb-6">
                                <label class="mb-2 block font-medium text-theme-dark/70" for="subject">Target Price</label>
                                <input class="form-control w-full" name="targetPrice" type="number" placeholder="Enter Target Price" id="targetPrice" />
                            </div>
                            <div class="mb-6">
                                <label class="mb-2 block font-medium text-theme-dark/70" for="subject">Lead Time</label>
                                <input class="form-control w-full" name="leadTime" type="date" placeholder="Enter Lead Time" id="leadTime" />
                            </div>
                            <div class="mb-6">
                                <label class="mb-2 block font-medium text-theme-dark/70" for="subject">Product Image</label>
                                <input class="form-control w-full" type="file" id="productImage" />
                            </div>
                            <input type="button" class="btn btn-primary mt-6 w-full" onclick="addRequirement()" value="Submit Now" />
                        </form>
                    </div>
                </div>
            </div>
    ...
    <script>
        function addRequirement() {
            ...
            formData.append('todaysDate', $('#todaysDate').val());
            formData.append('pieces', $('#pieces').val());
            formData.append('targetPrice', $('#targetPrice').val());
            formData.append('leadTime', $('#leadTime').val());

            let files = $('#productImage')[0].files;
            for (let i = 0; i < files.length; i++) {
                formData.append('images[]', files[i]);
            }

            $.ajax({
                url: "../api/insertBuyerRequirement.php",
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
                success: function(response) {
                    if (response.success == true)
                        window.location = "./buyerRequirements.php";
                    else {
                        window.location = "./addBuyerRequirement.php";
