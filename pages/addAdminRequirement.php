...
// $buyerRequirementId = $_POST["buyerId"];
$buyerRequirementId = $_GET["id"];

$query = "SELECT * FROM buyerrequirement WHERE Id = ?";
$params = [$buyerRequirementId];
...
<body><!-- page header -->
            ...
            <div class="row items-center justify-center">
                <div class="md:col-12 lg:col-6 aos-init aos-animate" data-aos="fade-up">
                    <div class="rounded-2xl p-8 pb-10 shadow">
                        <div class="mb-6">
                            <h3 class="h5 font-semibold">Make this live for Suppliers</h3>
                        </div>
                        <form action="#" method="post">
                            <div class="mb-6">
                                <label class="mb-2 block font-medium text-theme-dark/80" for="name">Product Name</label>
                                <input class="form-control shadow-none" id="name" type="text" required="" value="<?= $fetch["Name"] ?>">
                                <input type="hidden" value="<?= $fetch["Id"] ?>" id="buyerRequirementId">
                                <input type="hidden" value="<?= $fetch["BuyerId"] ?>" id="buyerId">
                            </div>
                            <div class="mb-6">
                                <label class="mb-2 block font-medium text-theme-dark/80" for="email">Product Description</label>
                                <input class="form-control shadow-none" id="description" type="text" required="" value="<?= $fetch["Description"] ?>">
                            </div>
                            <div class="mb-6">
                                <label class="mb-2 block font-medium text-theme-dark/80" for="email">Pieces</label>
                                <input class="form-control shadow-none" id="pieces" type="text" required="" value="<?= $fetch["Pieces"] ?>">
                            </div>
                            <div class="mb-6">
                                <label class="mb-2 block font-medium text-theme-dark/80" for="email">Target Price</label>
                                <input class="form-control shadow-none" id="targetPrice" type="text" required="" value="<?= $fetch["TargetPrice"] ?>">
                            </div>
                            <div class="mb-6">
                            ...
                            <div class="mb-6">
                                <label class="mb-2 block font-medium text-theme-dark/80" for="email">Count-Down</label>
                                <input class="form-control shadow-none" id="countDown" type="number" required="" placeholder="Enter your Count-Down: ">
                            </div>
                            <div class="mb-6">
                                <label class="mb-2 block font-medium text-theme-dark/80" for="email">Upload Date</label>
                                <input class="form-control shadow-none" id="todaysDate" type="date" required="" value="<?= $fetch["TodaysDate"] ?>">
                            </div>
                            <input type="button" value="Make Live" class="btn btn-primary mt-6 w-full" aria-label="Send Message" onclick="insertAdminRequirement()" />
                        </form>
                    </div>
                </div>
            </div>
    ...
    ?>
    <script>
        function insertAdminRequirement() {
            let data = {
            ...
            $.post("../api/insertAdminRequirement.php", data, function(response) {
                if (response.success == true)
                    window.location = "./allLiveRequirements.php";
                else
                    window.location = "./addAdmintRequirement.php";
